...
Dependencies are declared in ``pyproject.toml`` and pinned in ``uv.lock``. uv generates the lockfile, so **never edit it by hand**. Use the commands below and the two files stay in sync.

Runtime dependencies live under ``[project.dependencies]``. Optional and development packages live under ``[project.optional-dependencies]`` in the ``aws``, ``macOS``, ``webhooks``, and ``dev`` extras. Add a new package to the group that matches its purpose.

.. code-block:: console
    :caption: ``uv add`` and ``uv remove`` update both files and sync your environment automatically.

    % uv add <package>                    # runtime dependency
    % uv add --optional dev <package>     # add to an extra (dev, aws, macOS, webhooks)
    % uv remove <package>                 # remove a runtime dependency
    % uv remove --optional dev <package>  # remove from an extra

.. note::

    Use ``--optional <extra>`` to target an extra. uv's dependency groups are not used for this project, so avoid ``uv add --dev`` as it would create one.

If you change ``pyproject.toml`` by hand, run ``make lock`` to regenerate the lockfile to match. To upgrade pinned versions within the declared constraints:

.. code-block:: console

    % make upgrade                       # upgrade all packages, then sync
    % uv lock --upgrade-package <name>   # upgrade a single package

After a single-package upgrade, sync with ``make install``. Either way, commit both files together so every contributor and CI resolves the same versions.
